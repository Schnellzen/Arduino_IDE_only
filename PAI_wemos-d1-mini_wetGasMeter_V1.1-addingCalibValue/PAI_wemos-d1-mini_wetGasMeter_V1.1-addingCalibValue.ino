...
const float CALIBRATION_ML_PER_DEGREE = 0.42; // CHANGE THIS TO YOUR ACTUAL CALIBRATION VALUE
...
void updateOLED() {
  ...
  // Convert avgMLPS to string with appropriate decimal places
  String speedStr;
  
  if (fabs(avgMLPS) >= 1000.0) {
    speedStr = String(avgMLPS, 0);      // No decimals for ≥1000
  } else if (fabs(avgMLPS) >= 100.0) {
    speedStr = String(avgMLPS, 1);      // 1 decimal for 100-999
  } else if (fabs(avgMLPS) >= 10.0) {
    speedStr = String(avgMLPS, 2);      // 2 decimals for 10-99.9
  } else if (fabs(avgMLPS) >= 1.0) {
    speedStr = String(avgMLPS, 3);      // 3 decimals for 1-9.999
  } else {
    speedStr = String(avgMLPS, 4);      // 4 decimals for <1
  }
  
  // Calculate position for speed (top line)
  int speedStrLength = speedStr.length() + 4; // +4 for " ml/s"
  ...
  // Convert totalML to string
  String totalStr;
  
  if (fabs(totalML) >= 1000.0) {
    totalStr = String(totalML, 0);      // No decimals for ≥1000
  } else if (fabs(totalML) >= 100.0) {
    totalStr = String(totalML, 1);      // 1 decimal for 100-999
  } else if (fabs(totalML) >= 10.0) {
    totalStr = String(totalML, 2);      // 2 decimals for 10-99.9
  } else if (fabs(totalML) >= 1.0) {
    totalStr = String(totalML, 3);      // 3 decimals for 1-9.999
  } else {
    totalStr = String(totalML, 4);      // 4 decimals for <1
  }
  
  // Calculate position for total (second line)
  int totalStrLength = totalStr.length() + 3; // +3 for " ml"
  int totalX = (64 - (totalStrLength * 6)) / 2;
  
  // Display total on second line
  display.setCursor(totalX, 32);  // Y = 32 (middle of second 16 pixels)
...
void loop() {
  ...
  if (delta > 2048) {
    delta -= 4096;
    // totalML already handled by negative delta
  } else if (delta < -2048) {
    delta += 4096;
    // totalML already handled by positive delta
  }
